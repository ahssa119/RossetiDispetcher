...
Проанализируй текущие данные о погоде и местности и дай рекомендации.
...
- Перечисли 2-3 самых опасных риска для ЛЭП в данных условиях

📊 ВОЗДЕЙСТВИЕ НА ИНФРАСТРУКТУРУ (в текущий момент):
- Кратко опиши влияние текущей погоды на провода, опоры и оборудование
...
- 2-3 ключевых проверки и профилактических работы при данных условиях

⚠️ АВАРИЙНАЯ ГОТОВНОСТЬ (учитывая текущую погоду):
- Основные меры подготовки к возможным сбоям в данных условиях

Учти что это Вологодская область - характерны:
- Резкие изменения погоды
- Низкие температуры зимой
- Высокая влажность
- Сильные ветра

АНАЛИЗИРУЙ ТОЛЬКО ТЕКУЩУЮ ПОГОДУ, НЕ ДЕЛАЙ ПРОГНОЗОВ.

Будь конкретен и практичен! Ограничь ответ 2000 символами.
"""
        return prompt
    ...
    def _get_fallback_recommendations(self, weather_data: Dict, terrain_data: Dict, location: str) -> str:
        ...
        # Анализ рисков на основе текущих условий
        risks = []
...
🚨 ОСНОВНЫЕ РИСКИ (на основе текущих условий):
