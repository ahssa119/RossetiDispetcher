...
- Атмосферное давление: {current.get('pressure_msl', 'N/A')} гПа
- Облачность: {current.get('cloud_cover', 'N/A')}%
- Погода: {weather_description}
"""
        
        prompt = f"""
Ты - эксперт по энергетике и обслуживанию линий электропередач в Вологодской области. 
Проанализируй ТЕКУЩИЕ данные о погоде и дай рекомендации ТОЛЬКО по критическим рискам.

ВАЖНО: Используй только текущие погодные условия, не делай прогнозы на будущее.

{weather_info}

СФОРМУЛИРУЙ ОТВЕТ В СЛЕДУЮЩЕМ ФОРМАТЕ (МАКСИМУМ 2000 СИМВОЛОВ):

🚨 КРИТИЧЕСКИЕ РИСКИ (на основе текущих условий):
- Указывай ТОЛЬКО если есть: сильный порывистый ветер (скорость > 10 м/с или порывы > 15 м/с), грозовые явления (коды погоды 95, 96, 99), интенсивный снег (коды 71, 73, 75, 85, 86)
- Если этих явлений нет - пиши "Критических рисков нет"

📊 ВОЗДЕЙСТВИЕ НА ИНФРАСТРУКТУРУ (в текущий момент):
- Описывай ТОЛЬКО влияние снега и ветра, если они есть в текущих условиях
- Если снега и ветра нет или они слабые - пропускай этот раздел

👷‍♂️ РЕКОМЕНДАЦИИ БРИГАДАМ (для работы в текущих условиях):
- 3-4 самых важных меры безопасности и действий прямо сейчас

🔧 ТЕХНИЧЕСКИЕ МЕРОПРИЯТИЯ (на основе текущего состояния):
- ТОЛЬКО действия при аварии: порядок отключения, локализации, начала восстановительных работ
- Если критических рисков нет - укажи стандартные профилактические мероприятия

⚠️ АВАРИЙНАЯ ГОТОВНОСТЬ (учитывая текущую погоду):
- Основные меры подготовки к возможным сбоям в данных условиях

ПРАВИЛА:
1. Анализируй ТОЛЬКО текущую погоду
2. Не упоминай температуру, влажность, давление если они не критичны
3. Фокусируйся только на ветре, грозах и снеге
4. Будь конкретен и практичен
5. Ограничь ответ 2000 символами
6. Если критических погодных явлений нет - сообщи об этом
"""
        return prompt
    ...
    def _get_fallback_recommendations(self, weather_data: Dict, terrain_data: Dict, location: str) -> str:
        ...
        # Анализ критических рисков (только ветер, грозы, снег)
        critical_risks = []
        
        # Проверка ветра
        if wind > 10 or gusts > 15:
            critical_risks.append("💨 СИЛЬНЫЙ ПОРЫВИСТЫЙ ВЕТЕР")
        
        # Проверка грозовых явлений
        if weather_code in [95, 96, 99]:
            critical_risks.append("⚡ ГРОЗОВЫЕ ЯВЛЕНИЯ")
        
        # Проверка интенсивного снега
        if weather_code in [71, 73, 75, 85, 86]:
            critical_risks.append("❄️ ИНТЕНСИВНЫЙ СНЕГ")
        
        # Воздействие на инфраструктуру (только ветер и снег)
        infrastructure_impact = []
        if wind > 10:
            infrastructure_impact.extend([
                "• Механическая нагрузка на опоры и провода",
                "• Риск схлестывания проводов",
                "• Возможность падения деревьев на ЛЭП"
            ])
        if weather_code in [71, 73, 75, 85, 86]:
            infrastructure_impact.extend([
                "• Налипание снега на провода",
                "• Повышенная нагрузка на опоры",
                "• Ухудшение видимости для ремонтных работ"
            ])
        
        # Рекомендации бригадам
        recommendations = []
        if critical_risks:
            recommendations.extend([
                ...
                "• Ограничить высотные работы при сильном ветре"
            ])
        else:
            recommendations.extend([
                "• Стандартный режим патрулирования",
                "• Плановые профилактические работы",
                "• Контроль состояния оборудования"
            ])
        
        # Технические мероприятия (только при аварии)
        technical_measures = []
        if critical_risks:
            technical_measures.extend([
                "• Немедленное отключение поврежденного участка",
                "• Локализация зоны аварии",
                "• Начало восстановительных работ после стабилизации погоды"
            ])
        else:
            technical_measures.extend([
                "• Плановые профилактические осмотры",
                "• Контроль натяжения проводов",
                "• Проверка креплений опор"
            ])
...
• Погода: {self._decode_weather_code(weather_code)}

🚨 КРИТИЧЕСКИЕ РИСКИ:
{chr(10).join(f'- {risk}' for risk in critical_risks) if critical_risks else '- Критических рисков нет'}

📊 ВОЗДЕЙСТВИЕ НА ИНФРАСТРУКТУРУ:
{chr(10).join(infrastructure_impact) if infrastructure_impact else '- Значительного воздействия нет'}

👷‍♂️ РЕКОМЕНДАЦИИ БРИГАДАМ:
{chr(10).join(recommendations)}

🔧 ТЕХНИЧЕСКИЕ МЕРОПРИЯТИЯ:
{chr(10).join(technical_measures)}

⚠️ Примечание: нейросеть временно недоступна.
"""
        
        # Ограничиваем резервные рекомендации 2000 символами
...
🤖 Использую нейросеть GPT-4 для анализа ТЕКУЩИХ погодных условий

Я анализирую ТЕКУЩУЮ погоду в центрах муниципальных округов и даю рекомендации для:
...
📍 Просто напишите название города, поселка или села
"""
...
1. Отправьте название центра муниципального округа Вологодской области
   Например: "Вологда", "Великий Устюг", "Липин Бор", "Шексна"

2. Бот запросит:
   • Текущие данные о погоде (OpenMeteo)
   • Информацию о местности (OpenStreetMap)

3. Нейросеть GPT-4 сгенерирует:
   • Анализ рисков для ЛЭП на основе текущих условий
   • Рекомендации для бригад для работы прямо сейчас
   • Технические мероприятия с учетом актуальной погоды
"""
        ...
        for name, data in VOLOGDA_REGION_LOCATIONS.items():
            if len(name) > 3 and not name.startswith(('им ', 'великийустюг', 'кичменгский', 'тарногский', 'бабушкина')):
                if data['type'] == 'город':
                    ...
                    cities_by_type["Поселки"].append(name.title())
                elif data['type'] == 'село':
                    cities_by_type["Села"].append(name.title())
    ...
    def find_similar_locations(self, user_input: str) -> List[str]:
        ...
        for location in VOLOGDA_REGION_LOCATIONS.keys():
            if (user_input in location or 
                ...
                if location in ["имени бабушкина", "им бабушкина"]:
                    display_name = "им. Бабушкина"
                elif location == "кичменгский городок":
                    display_name = "Кичменгский Городок"
                elif location == "тарногский городок":
    ...
    async def analyze_location(self, update: Update, location_name: str):
        ...
        if location_name in ["имени бабушкина", "им бабушкина"]:
            display_name = "им. Бабушкина"
        elif location_name == "кичменгский городок":
            display_name = "Кичменгский Городок"
...
def main():
    ...
    print("🏘 Поселки: 4") 
    print("🏡 Села: 9")
    print("🤖 Используется нейросеть: GPT-4")
    print("🌤️ Анализ: только ТЕКУЩИЕ погодные условия")
    print("📝 Ограничение ответов: 2000 символов")
